document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        const pendingOrders = orders.filter(order => order.shipmentStatus.status === "Delivered" || order.shipmentStatus.status === "delivered");

        if (pendingOrders.length === 0) {
            ordersContainer.innerHTML = "<p>No orders delivered yet</p>";
            return;
        }
